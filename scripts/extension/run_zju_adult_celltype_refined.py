...
def main():
    ...
    ap.add_argument('--out-root', default=str(ROOT / 'results' / 'zju_cellatlas_adult_celltype_refined_matching_by_context'))
    args = ap.parse_args()
    ...
    inv = pd.read_csv(ROOT / 'results' / 'extension_forward_visibility' / 'data_inventory' / 'zju_cellatlas_adult_celltype_contexts.csv')
    row = inv[(inv['status'].eq('prepared')) & (inv['analysis_level'].eq('adult_celltype')) & (inv['context'].eq(args.context))]
    if row.empty:
        raise SystemExit(f'No prepared adult celltype context found: {args.context}')
    r = row.iloc[0]
    config = {
        'dataset': 'ZJU_HCL_adult_celltype',
        'condition': str(r['context']),
        'h5ad': resolve_project_path(r['h5ad']),
        'gene_summary': resolve_project_path(r['gene_summary']),
        'edge_sets': edge_sets,
    }
    out_dir = Path(args.out_root) / f"adult_celltype__{slug(args.context)}"
